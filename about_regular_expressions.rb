...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    assert_equal "a", "aabbcccddddeeeee"[/az*/]
  ...
  def test_character_classes_give_options_for_a_character
    ...
    # puts animals.select { |a| a[/at/] }
    #[] makes them options of or with anything following
    assert_equal ["cat", "bat", "rat"], animals.select { |a| a[/[cbr]at/] }

  end

  def test_slash_d_is_a_shortcut_for_a_digit_character_class
    #one of more of any of the numbers
    assert_equal "42", "the number is 42"[/[0123456789]+/]
    assert_equal nil, "the number is"[/[0123456789]+/]
  ...
  def test_slash_w_is_a_shortcut_for_a_word_character_class
    ...
    #i'm not sure why this isnt "variable_142"
    assert_equal "variable_1", "variable_1 = 42"[/[a-zA-Z0-9_]+/]
    assert_equal "variable_1", "variable_1 = 42"[/\w+/]
  end

  #confused about this 
  #/./ Any character except a newline. stops after new line?
  ...
  def test_caret_anchors_to_the_start_of_lines
    #^ means start of and \d means number
    assert_equal "2", "num 42\n2 lines"[/^\d+/]
  end

  def test_dollar_sign_anchors_to_the_end_of_lines
    #$ means at the end of lines \d means any number and + means more than one
    assert_equal "42", "2 lines\nnum 42"[/\d+$/]
  end

  def test_slash_b_anchors_to_a_word_boundary
    #needs to be a new word
    assert_equal "vines", "bovine vines"[/\bvine./]
    #only one cuz greedy
    assert_equal "vinel", "bovine vinel vines"[/\bvine./]
  ...
  #Alternations work on entire regex captures while character class only works on characters
  # ------------------------------------------------------------------
